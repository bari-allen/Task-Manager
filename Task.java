 ...
 *
 * @author Michelle & Karl Haidinyak
 */
public class Task {
  ...
  /** denotes whether this task has been completed*/
   ...
   * @return &lt; 0 if this is smaller than other, &gt; zero of this is larger than other, and 0 if
   * they are equal
   *
   * @author karl haidinyak
   */
  public int compareTo(Task other, CompareCriteria criteria) {
    if (criteria.equals(CompareCriteria.TITLE)) {
      return -1 * this.title.compareTo(other.title);
    }
    if (criteria.equals(CompareCriteria.TIME)) {
      int timeDifference = getTime() - other.getTime();
      return timeDifference;
    }
    if (criteria.equals(CompareCriteria.LEVEL)) {
